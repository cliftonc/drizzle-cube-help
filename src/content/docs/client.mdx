...
<div class="client-hero-grid">
  <div class="client-hero-card">
    <a href="/client/analysis-builder/" class="lightbox-trigger">
      <Image src={analysisHero} alt="Analysis Builder - Interactive query builder" class="client-hero-image" />
    </a>
    <div class="client-hero-info">
      <h3><a href="/client/analysis-builder/">Analysis Builder</a></h3>
      <p>Interactive query builder with auto-execute, AI generation, and shareable URLs</p>
    </div>
  </div>
  <div class="client-hero-card">
    <a href="/client/dashboards/" class="lightbox-trigger">
      <Image src={dashboardHero} alt="Dashboards - Drag-and-drop analytics" class="client-hero-image" />
    </a>
    <div class="client-hero-info">
      <h3><a href="/client/dashboards/">Dashboards</a></h3>
      <p>Drag-and-drop layouts with configurable portlets and persistence</p>
    </div>
  </div>
</div>
...
function CustomAnalytics() {
  ...
  const { meta } = useCubeMetaQuery();

  // Build your own UI with the data
  return (
    <div>
      <div>Available cubes: {meta?.cubes?.length ?? 0}</div>
      {data?.map(row => (
...
| `showSchemaDiagram` | `boolean` | `false` | Show interactive [schema visualization](/client/schema-visualization) button in AnalysisBuilder results panel (requires `@xyflow/react` and `elkjs`) |
...
```tsx
import { validateCubeQuery } from 'drizzle-cube/client';

function AnalysisBuilder({ query, onChange }) {
  const validation = validateCubeQuery(query);
...
- [**Analysis Builder**](/client/analysis-builder/) - Interactive query builder
