...
Rows are offered under aliases because Knap reads `a.b` as nested member access. A cube-qualified key such as `Employees.count` is therefore unreachable from a loop or from the `map` filter, and only bracket syntax on `data` gets at it directly.

Two further limits are worth knowing before you write a long template. A filter chain returns a string once assigned, so a template cannot sort rows and then index into the result — order the query itself and read `first` or `last`. And `rows | first` renders an object rather than a field, so reach into a row with `rows[0].employees_count`.
