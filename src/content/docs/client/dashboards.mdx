...
<div class="dashboard-landing-split">
  <div class="dashboard-landing-image-col lightbox-trigger">
    <Image src={dashboardHero} alt="Drizzle Cube Dashboard - Embeddable analytics dashboard with drag-and-drop layout" class="dashboard-landing-image" />
  </div>
  <div class="dashboard-landing-content">
    <p>Embed production-ready analytics dashboards into your application with minimal code. Built on React and react-grid-layout, Drizzle Cube dashboards integrate directly with your semantic layer—giving your users the power to create, customize, and save their own dashboard layouts without backend changes.</p>
    <ul class="dashboard-quick-links">
      <li><a href="/client/charts/">Charts</a> — Bar, line, area, pie, and more</li>
      <li><a href="/client/dashboard-persistence/">Persistence</a> — Save & load with your backend</li>
      <li><a href="/client/analysis-builder/">Analysis Builder</a> — Query building UI</li>
      <li><a href="/client/theming/">Theming</a> — Customize appearance</li>
      <li><a href="/semantic-layer/security/">Security</a> — Multi-tenant isolation</li>
      <li><a href="/getting-started/quick-start/">Quick Start</a> — Get started fast</li>
    </ul>
  </div>
</div>

### Why Drizzle Cube Dashboards?

- **Type-safe configuration** — Full TypeScript support with `DashboardConfig` and `PortletConfig` interfaces
- **Semantic layer integration** — Queries use your cube definitions, ensuring consistent business logic
- **Drag-and-drop layouts** — Users can rearrange, resize, and customize their view
- **Flexible layout modes** — Grid-based or row-based layouts to match your UX patterns
- **Built-in persistence** — Save and restore dashboard configurations with your own backend
- **Multi-tenant ready** — Security context flows through from your semantic layer
- **Modular imports** — Import only what you need (~218KB for dashboard-only builds)
...
import { AnalysisBuilder, PortletAnalysisModal } from 'drizzle-cube/client';
...
#### Customizing or replacing the toolbar

`DashboardGrid` renders its own toolbar. To suppress it, pass `hideToolbar` (this hides both the top edit bar and the floating toolbar):

```tsx
<DashboardGrid config={config} editable hideToolbar onSave={save} />
```

To render your **own** toolbar with your design system while Drizzle Cube keeps owning the edit/save state machine — or to embed a grid-only / filter-only view — compose the underlying pieces (`DashboardProvider`, `DashboardToolbar`, `DashboardFilterBar`, `DashboardGridSurface`, `DashboardModals`) and read state via `useDashboardContext()`. See [Composable Dashboards](/client/composable-dashboards/).
...
Built-in modal for configuring portlets (uses the modern Analysis Builder):

```tsx
import { PortletAnalysisModal } from 'drizzle-cube/client'
...
- Build a custom toolbar or grid-only embed with [Composable Dashboards](/client/composable-dashboards/)
